...
public class ReservationService {
    ...
    /**
     * 特定の日付と期間セットの予約詳細を確認します
     *
     * @param pc_id 可用性を確認する対象のエンティティ（機器や場所など）のID
     * @param date 予約を確認している日付
     * @param otherPurpose 予約に関連する任意の目的または追加情報
     * @param periods チェックするタイムスロットを表す期間のリスト
     * @return 予約の詳細（期間グループ、開始時刻、終了時刻など）を含む
     *          {@code ReservationCheckDTO} オブジェクト
     */
     ...
     */
    private List<List<Byte>> getGroups(List<Byte> periodInts) {
